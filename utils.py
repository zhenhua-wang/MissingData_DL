...
def onehot(data):
  ...
  return data_x_enc, n_classes

def reverse_onehot(data_enc, n_classes):
  data = data_enc[:, 0:n_classes[0]].argmax(axis = 1)
  current_ind = n_classes[0]
  for n in n_classes[1:]:
    data = np.column_stack((data, data_enc[:, current_ind:current_ind + n].argmax(axis = 1)))
    current_ind += n
  return data
